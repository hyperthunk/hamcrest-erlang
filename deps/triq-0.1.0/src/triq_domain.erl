...
-export([list/1, tuple/1, int/0, real/0, sized/1, elements/1, any/0, atom/0, choose/2,
	 oneof/1, boolean/0, char/0, string/0, return/1, vector/2, binary/1]).
...
component_domain(_, #?DOM{kind={list,ElemDom}}) ->
    ElemDom;
component_domain(_, #?DOM{kind={vector,ElemDom,Len}}) ->
    ElemDom;
...
%% @doc Returns the domain of character strings.
%% @spec string() -> domain()
string() ->
	list(char()).
...
vector(Len, G) when Len > 0, is_integer(Len) ->
